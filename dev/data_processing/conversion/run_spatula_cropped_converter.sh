...
# Configuration
BASE_DIR="/root/lerobot_ws/spatula"
OUTPUT_DIR="/root/lerobot_ws/spatula_cropped"
MAPPING_FILE=""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    python rosbag_to_lerobot_cropped_trimmed.py \
        ...
        --dataset-name "spatula_cropped" \
        --fps 20 \
        --task "Spatula task with dual cameras and left arm control cropped" \
        --tolerance 1.0 \
        --no-trim-unmoving \
        --no-downsize \
