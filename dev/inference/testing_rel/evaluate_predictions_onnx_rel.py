...
class ONNXTensorRTInference:
    ...
    def __init__(self, checkpoint_path: str, onnx_dir: str, device: str = "cuda"):
        ...
        self.target_size = (224, 224)  # (width, height) - final size for both cameras
    ...
    def predict(self, left_image, head_image, joint_state) -> np.ndarray:
        ...
        if len(self._queues["action"]) == 0:
            ...
            if self.use_relative_actions:
                ...
                state_batch[:, :, : self.arm_dim] = (
                    state_batch[:, :, : self.arm_dim] - current_state
                )
            ...
            if self.use_relative_actions:
                ...
                action_chunk_unnorm[:, :, : self.arm_dim] = (
                    action_chunk_unnorm[:, :, : self.arm_dim]
                    + state_stored[: self.arm_dim]
                )
...
class PredictionEvaluator:
    ...
    def load_rosbag_data(
        self, rosbag_path: str, max_samples: int = 100
    ) -> Dict[str, List]:
        ...
        storage_options = rosbag2_py.StorageOptions(
            uri=str(rosbag_path), storage_id="mcap"
        )
    ...
    def evaluate(self, rosbag_path: str, num_samples: int = 50) -> Dict:
        ...
        print(
            f"Loaded messages - Left: {left_count}, Head: {head_count}, Joints: {joint_count}, Actions: {action_count}"
        )
        ...
        print(
            f"Using DDIM scheduler with {self.inference.config['num_inference_steps']} steps"
        )
        ...
        for i in range(num_samples):
            try:
                ...
                pred = self.inference.predict_from_ros_messages(
                ...
                if (i + 1) % 10 == 0:
                    ...
                    print(
                        f"Processed {i + 1}/{num_samples} samples - Avg inference: {avg_time:.2f} ms"
                    )
    ...
    def print_results(self, results: Dict):
        ...
        print(f"\nOverall Metrics:")
        print(
            f"  MAE:  {results['mae_overall']:.4f} rad ({np.rad2deg(results['mae_overall']):.2f}°)"
        )
        print(
            f"  RMSE: {results['rmse_overall']:.4f} rad ({np.rad2deg(results['rmse_overall']):.2f}°)"
        )
        print(
            f"  Max Error: {results['max_error_overall']:.4f} rad ({np.rad2deg(results['max_error_overall']):.2f}°)"
        )
        print(
            f"  Std Error: {results['std_error_overall']:.4f} rad ({np.rad2deg(results['std_error_overall']):.2f}°)"
        )
        ...
        for i, joint_name in enumerate(self.left_arm_joints):
            print(f"\n  {joint_name}:")
            print(
                f"    MAE:  {results['mae_per_joint'][i]:.4f} rad ({np.rad2deg(results['mae_per_joint'][i]):.2f}°)"
            )
            print(
                f"    RMSE: {results['rmse_per_joint'][i]:.4f} rad ({np.rad2deg(results['rmse_per_joint'][i]):.2f}°)"
            )
            print(
                f"    Max:  {results['max_error_per_joint'][i]:.4f} rad ({np.rad2deg(results['max_error_per_joint'][i]):.2f}°)"
            )
            print(
                f"    Std:  {results['std_error_per_joint'][i]:.4f} rad ({np.rad2deg(results['std_error_per_joint'][i]):.2f}°)"
            )

        print("\nOptimization Applied:")
        print(f"  - Backend: ONNX Runtime with TensorRT")
        print(
            f"  - Scheduler: DDIM with {self.inference.config['num_inference_steps']} inference steps"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate ONNX+TensorRT predictions (Relative Action Support)"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--num-samples", type=int, default=200, help="Number of samples to evaluate"
    )
