...
class InferenceNode(Node):
    ...
    def __init__(
        ...
        self.action_pub = self.create_publisher(
            JointTrajectory, "/left_arm/joint_trajectory", control_qos
        )
    ...
    def publish_action(self, action: np.ndarray):
        ...
        if self.inference.output_mode == "pos_only":
            ...
            if len(action) != 6:
                raise ValueError(
                    f"Expected 6D action for pos_only mode, got {len(action)}D"
                )
            ...
            if len(action) != 12:
                raise ValueError(
                    f"Expected 12D action for pos_vel mode, got {len(action)}D"
                )
            positions = action[:6].astype(np.float64).tolist()
            velocities = action[6:].astype(np.float64).tolist()
            self.get_logger().debug(
                f"Position+velocity action - Pos: {positions}, Vel: {velocities}"
            )
        else:
...
def main():
    ...
    try:
        ...
        print(
            "LeRobot ONNX Inference Node started (Approach Plate - Relative Action Mode)"
        )
