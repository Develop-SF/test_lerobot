...
# Add testing_abs directory to path to import LeRobotInference
# We assume this script is in .../testing_rel/
# And lerobot_inference.py is in .../testing_abs/
current_dir = Path(__file__).parent
testing_abs_dir = current_dir.parent / "testing_abs"
sys.path.append(str(testing_abs_dir))
...
class PredictionEvaluator:
    ...
    def extract_ground_truth_action(self, action_msg) -> np.ndarray:
        """Extract commanded positions from JointTrajectory message."""
        if not hasattr(action_msg, 'points') or not action_msg.points:
            raise ValueError("JointTrajectory message has no trajectory points")

        first_point = action_msg.points[0]
        ...
        command_positions = np.array(first_point.positions, dtype=np.float32)
    ...
    def evaluate(self, rosbag_path: str, num_samples: int = 50) -> Dict:
        ...
        for i in range(num_samples):
            try:
                ...
                # Extract ground truth (commanded action)
                # For evaluation, we compare predicted action with commanded action
                gt_positions = self.extract_ground_truth_action(action_msg)
