#!/usr/bin/env python3
"""
Convert Approach Real Bing 20Hz LeRobot Diffusion Policy to ONNX format (Relative Action Support).
...
Supports relative action representation.
...
# Assuming script is in: workspace/approach_real_bing_20hz_lerobot/testing_rel/convert_to_onnx_rel.py
...
def export_rgb_encoder(policy, output_dir: Path, opset_version: int = 17):
    ...
    if crop_shape:
        ...
        # Use target size from lerobot_inference.py if not in config
        # Target size is (width, height) = (224, 178) -> (height, width) = (178, 224)
        # BUT wait, lerobot_inference.py says target_size = (224, 178) (width, height)
        # So height is 178, width is 224.
        # Let's check if we can get it from the model config first.
        # If not, fallback to the known target size.
        input_features = config_dict.get("input_features", {})
        # Try to find a shape in input features
        found_shape = None
        for key, value in input_features.items():
            if "images" in key and isinstance(value, dict) and "shape" in value:
                found_shape = value["shape"] # (C, H, W)
                break
        
        if found_shape:
             crop_h, crop_w = found_shape[1], found_shape[2]
        else:
             # Fallback to hardcoded values from lerobot_inference.py
             # target_size = (224, 178) -> (width, height)
...
def main():
    parser = argparse.ArgumentParser(description="Convert Approach Real Bing 20Hz model to ONNX (Relative Action Support)")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="approach_real_bing_20hz_lerobot/testing_rel/onnx_models",
    ...
    print("="*80)
    print("APPROACH REAL BING 20HZ MODEL - ONNX CONVERSION (RELATIVE)")
    print("="*80)
    ...
    with torch.no_grad():
        ...
        config_path = save_config(policy, output_dir, args.use_relative_actions, args.arm_dim)
    ...
    print(f"     python evaluate_predictions_onnx_rel.py \\")
    print(f"         --checkpoint {args.checkpoint} \\")
