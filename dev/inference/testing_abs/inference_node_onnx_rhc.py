...
Absolute Action Mode WITH Receding Horizon Control (RHC) / Smoothing

This node subscribes to sensor topics, runs inference using ONNX Runtime with TensorRT,
and publishes actions.

Key Features:
- Async inference in background thread
- Action aggregation (weighted averaging) for smoothing
- Adaptive replanning based on queue depth
...
class ONNXTensorRTInference:
    """ONNX Runtime with TensorRT backend inference for Approach Real Bing 20Hz model (Absolute) with RHC."""
